...
class AnimatedList extends StatefulWidget {
  ...
  const AnimatedList({
    ...
    @required this.prependItemBuilder,
    this.initialItemCount = 0,
    this.prependItemCount = 0,
  ...
  final NullableIndexedWidgetBuilder prependItemBuilder;
...
class AnimatedListState extends State<AnimatedList>
  ...
  final GlobalKey<SliverAnimatedListState> _prependSliverAnimatedListKey =
      GlobalKey();
  final Key centerKey = UniqueKey();
  final Box<SliverConstraints> leadingConstraints = Box();
  double _endOffset = 0;
  ...
  void insertItem(int index, {Duration duration = _kDuration}) {
    layoutGroup.notifyListeners();
    _sliverAnimatedListKey.currentState.insertItem(index, duration: duration);
  }

  void prependItem(int index, {Duration duration = _kDuration}) {
    _prependSliverAnimatedListKey.currentState
  ...
  final layoutGroup = ChangeNotifier();
  @override
  Widget build(BuildContext context) {
    return CustomScrollView(
      ...
      // anchor: (widget.prependItemCount.toDouble() / 11.0).clamp(0.0, 1.0),
      anchor: 1.0,
      center: centerKey,
      slivers: <Widget>[
        SliverFill(
          child: const SizedBox(),
          selfConstraints: leadingConstraints,
        ),
        SliverPadding(
          padding: widget.padding ?? EdgeInsets.zero,
          sliver: SliverList(
            key: _prependSliverAnimatedListKey,
            delegate: SliverChildBuilderDelegate(widget.prependItemBuilder,
                childCount: widget.prependItemCount),
          ),
        ),
        SliverPadding(
          key: centerKey,
        ...
        SliverFill(
            ...
            constraintsAdapter: (constraints) {
              _endOffset = constraints.precedingScrollExtent;
              print('endOffset = ${_endOffset}');
              final leading =
                  leadingConstraints.value?.precedingScrollExtent ?? 0;
              final height = (constraints.viewportMainAxisExtent -
                      leading -
            ...
            child: Container(color: Colors.amber, child: Text('hehehehe'))),
      ],
    );
  }
}
...
class SliverFill extends SingleChildRenderObjectWidget {
  final Box<SliverConstraints> selfConstraints;
  final BoxConstraints Function(SliverConstraints c) constraintsAdapter;
  final ChangeNotifier layoutGroup;
  final VoidCallback onLayout;
  SliverFill(
      {Key key,
      Widget child,
      this.selfConstraints,
      this.constraintsAdapter,
      this.layoutGroup,
      this.onLayout})
      : super(key: key, child: child);
  @override
  RenderObject createRenderObject(BuildContext context) {
    return RenderSliverFill();
  }

  @override
  void updateRenderObject(
      BuildContext context, covariant RenderSliverFill renderObject) {
    super.updateRenderObject(
      context,
      renderObject
        ..selfConstraints = selfConstraints
        ..constraintsAdapter = constraintsAdapter
        ..onLayout = onLayout
        ..layoutGroup = layoutGroup,
    );
  }
...
class RenderSliverFill extends RenderSliverSingleBoxAdapter {
  Box<SliverConstraints> selfConstraints;
  BoxConstraints Function(SliverConstraints c) constraintsAdapter;
  VoidCallback _layoutDisposable;
  VoidCallback onLayout;
  double _lastExtent;
  set layoutGroup(ChangeNotifier v) {
    _layoutDisposable?.call();
    _layoutDisposable = v?.addListenerDisposable(() {
      markNeedsLayout();
    });
  }
  ...
  void performLayout() {
    ...
    selfConstraints?.value = constraints;
    if (child == null) {
      geometry = SliverGeometry.zero;
      return;
    }
    child.layout(
        constraintsAdapter?.call(constraints) ?? constraints.asBoxConstraints(),
        parentUsesSize: true);
    double childExtent = 0;
    ...
    double corr;
    if (_lastExtent != null && _lastExtent != childExtent) {
      corr = -_lastExtent + childExtent;
    }
    ...
    geometry = SliverGeometry(
      ...
      // scrollOffsetCorrection: corr
    );
    _lastExtent = childExtent;
